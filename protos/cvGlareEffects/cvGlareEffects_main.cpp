...
int main( int argc, char* argv[] )
{
   ...
   if ( lf_scale > 1.0 )
   {
      Size roi_size( K.cols / lf_scale, K.rows / lf_scale );
      Point2f roi_center( 0.5 * K.cols, 0.5 * K.rows );
      Mat KTransformed(
          K,
          Rect(
              roi_center.x - 0.5 * roi_size.width,
              roi_center.y - 0.5 * roi_size.height,
              roi_size.width,
              roi_size.height ) );
